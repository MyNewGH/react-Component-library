...
* */
function randamStr(flag:boolean,min:number=0,max?:number) {
  let str='',rang=min,strs='0123456789abcdefghijklmnopqrstuvwxyz';
  if (flag&&max){
    rang = Math.round(Math.random()*(max-min))+min;
  }
  for (let i=0;i<rang;i++){
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    str+= strs[Math.round(Math.random()*strs.length-1)]
  }
  return str
}
